...
use crate::BlockGetter;
use crate::EntityDimensions;

use crate::collision::{collect_obstacles, collide_with_shapes};
use crate::constants::*;
use crate::voxel_shape::BlockShapeProvider;
...
pub fn physics_tick(
    ...
    let block_friction = get_block_friction(level, pos.x, bbox.min_y, pos.z, *on_ground);

    // 2. Apply gravity (or fluid modifiers).
    if in_water {
        vel.y += WATER_BUOYANCY;
    } else if in_lava {
        vel.y += LAVA_BUOYANCY;
    } else {
        vel.y -= GRAVITY;
    ...
    let speed_factor = get_block_speed_factor(level, pos.x, bbox.min_y, pos.z);
    vel.x *= speed_factor;
    vel.z *= speed_factor;
    ...
    let new_x = pos.x + actual_dx;
    let new_y = pos.y + actual_dy;
    let new_z = pos.z + actual_dz;
    pos.x = new_x;
    pos.y = new_y;
    pos.z = new_z;
    *bbox = Aabb::from_center(
        new_x,
        new_y,
        new_z,
        f64::from(dims.width),
    ...
    *on_ground = dy < 0.0 && (actual_dy - dy).abs() > COLLISION_EPSILON;
    ...
    if (actual_dy - dy).abs() > COLLISION_EPSILON {
        ...
        if *on_ground && is_on_slime(level, pos.x, bbox.min_y, pos.z) {
            vel.y = -vel.y;
        } else {
            vel.y = 0.0;
        }
...
mod tests {
    ...
    use super::*;
    use crate::voxel_shape::FullCubeShapeProvider;
    use crate::{BlockAccessError, EntityDimensions};
    use glam::DVec3;
    ...
    impl BlockGetter for EmptyLevel {
        fn get_block_state(&self, _pos: BlockPos) -> Result<u32, BlockAccessError> {
            Ok(0) // air
        }
    }
    ...
    fn make_floating_state() -> (DVec3, DVec3, Aabb, bool, f32) {
        ...
        (
            p,
            DVec3::ZERO,
            Aabb::from_center(p.x, p.y, p.z, 0.6, 1.8),
            false,
            0.0f32,
        )
    }

    fn make_above_floor_state(height: f64) -> (DVec3, DVec3, Aabb, bool, f32) {
    ...
    fn test_collision_stops_downward_movement() {
        ...
        assert!(og, "Entity should be on ground");
        assert!(vel.y.abs() < 0.001, "vy should be zeroed: {}", vel.y);
        assert!(
            pos.y >= 1.0 - 0.001,
    ...
    fn test_water_buoyancy() {
        ...
        assert!(vel.y > 0.0, "Water buoyancy should push upward");
        assert!(vel.x < 1.0, "Water should reduce horizontal velocity");
    }
    ...
    fn test_fall_distance_accumulates() {
        ...
        assert!(fd > 0.0, "Fall distance should increase while falling");
        ...
        assert!(fd > fd1, "Fall distance should keep increasing");
    }
    ...
    fn test_entity_at_rest_on_ground() {
        ...
        assert!(og, "Entity should remain on ground");
        assert!(
            (pos.y - y_before).abs() < 0.01,
    ...
    fn test_single_block_collision() {
        ...
        let mut pos = p;
        let mut vel = DVec3::new(0.0, -5.0, 0.0);
        let mut bbox = Aabb::from_center(p.x, p.y, p.z, 0.6, 1.8);
        let mut og = false;
        let mut fd = 0.0f32;
        ...
        assert!(og, "Entity should land on single block");
        assert!(
            pos.y >= 1.0 - 0.001,
    ...
    fn test_slime_bounce() {
        ...
        assert!(og, "Entity should land on slime");
        assert!(
            vel.y > 0.0,
